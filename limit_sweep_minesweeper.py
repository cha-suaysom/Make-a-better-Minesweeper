import random


class boardSpot(object):
    value_1 = 0
    value_2 = 0
    selected = False
    mine = False
    distanceSelected = 0

    def __init__(self):
        self.selected = False

    def __str__(self):
        return str(boardSpot.value)


class boardClass(object):
    def __init__(self, m_boardSize, m_numMines):
        self.board = [[boardSpot() for i in range(m_boardSize)] for j in range(m_boardSize)]
        self.boardSize = m_boardSize
        self.numMines = m_numMines
        self.selectableSpots = m_boardSize * m_boardSize - m_numMines
        i = 0
        while i < m_numMines:
            x = random.randint(0, self.boardSize-1)
            y = random.randint(0, self.boardSize-1)
            if not self.board[x][y].mine:
                self.addMine(x, y)
                i += 1
            else:
                i -= 1

    def __str__(self):
        returnString = " "
        divider = "\n---"

        for i in range(0, self.boardSize):
            returnString += " |   " + str(i) + "   " #First line
            divider += "---------"
        divider += "\n"

        returnString += divider
        for y in range(0, self.boardSize):
            returnString += str(y)
            for x in range(0, self.boardSize):
                distance = self.board[x][y].distanceSelected
                if (distance == 1):
                    boardSpotValue = self.board[x][y].value_1
                if (distance == 2):
                    boardSpotValue = self.board[x][y].value_2

                if self.board[x][y].mine and self.board[x][y].selected:
                    returnString += " |" + str((boardSpotValue,distance))
                elif self.board[x][y].selected:
                    returnString += " | " + str((boardSpotValue,distance))
                else:
                    returnString += " |       "
            returnString += " |"
            returnString += divider #ignore
        return returnString

    def addMine(self, x, y):
        self.board[x][y].value_1 = -1
        self.board[x][y].value_2 = -1
        self.board[x][y].mine = True
        #print("Finish adding mines at spot", (x,y))

        #Dominate 1
        for i in range(x-1, x+2):
            if i >= 0 and i < self.boardSize:
                if y-1 >= 0 and not self.board[i][y-1].mine:
                    self.board[i][y-1].value_1 += 1
                if y+1 < self.boardSize and not self.board[i][y+1].mine:
                    self.board[i][y+1].value_1 += 1
        if x-1 >= 0 and not self.board[x-1][y].mine:
            self.board[x-1][y].value_1 += 1
        if x+1 < self.boardSize and not self.board[x+1][y].mine:
            self.board[x+1][y].value_1 += 1

        #Dominate 2
        j = 2 
        for colIndex in range(x-j,x+j+1):
            for rowIndex in range(y-j,y+j+1):
                if ((0 <= colIndex) and (colIndex < self.boardSize)):
                    if ((0 <= rowIndex) and (rowIndex < self.boardSize)):
                        if (not self.board[colIndex][rowIndex].mine):
                            (self.board[colIndex][rowIndex].value_2) +=1
        #print("The value at spot is")
        #print(self.board[x][y].value1)
        #print(self.board[x][y].value2)

    def makeMove(self, x, y,distance):
        self.board[x][y].distanceSelected = distance
        self.board[x][y].selected = True
        self.selectableSpots -= 1
        if (distance == 1):
            boardSpotValue = self.board[x][y].value_1
        if (distance == 2):
            boardSpotValue = self.board[x][y].value_2

        if boardSpotValue == -1:
            return False
        if boardSpotValue == 0:
            for i in range(x-1, x+2):
                if i >= 0 and i < self.boardSize:
                    if y-1 >= 0 and not self.board[i][y-1].selected:
                        self.makeMove(i, y-1,distance)
                    if y+1 < self.boardSize and not self.board[i][y+1].selected:
                        self.makeMove(i, y+1,distance)
            if x-1 >= 0 and not self.board[x-1][y].selected:
                self.makeMove(x-1, y, distance)
            if x+1 < self.boardSize and not self.board[x+1][y].selected:
                self.makeMove(x+1, y, distance)
            return True
        else:
            return True

    def hitMine(self, x, y):
        return (self.board[x][y].value_1) == -1

    def isWinner(self):
        return self.selectableSpots == 0


#play game
def playGame():
    # boardSize = int(input("Choose the Width of the board: "))
    # numMines = int(input("Choose the number of mines: "))

    # numDis1 = int(input("Limit Distance 1 sweep "))
    # numDis2 = int(input("Limit Distance 2 sweep "))

    boardSize = 10
    numMines = 5

    disList = [5,20]

    gameOver = False
    winner = False
    Board = boardClass(boardSize, numMines)
    show = True
    while not gameOver:
        if (show == True):
            print(Board)
        else:
            show = True
        print("You can do distance 1 sweep " + str(disList[0]) + " times")
        print("You can do distance 2 sweep " + str(disList[1]) + " times")
        print("Make your move:")
        x = int(input("x: "))
        y = int(input("y: "))
        dis = int(input("dis: "))

        if ((dis != 1) and (dis != 2)):
            print(" ")
            print("Unfortunately we only support distance 1,2 sweep right now. Please select the correct distance")
            print("No move has been made.")
            show = False
            continue

        if (disList[dis-1] == 0):
            print("You have reach sweep limit for distance. No move has been made. " + dis)
            show = False
        else:
            disList[dis-1] -= 1
            Board.makeMove(x, y, dis)




        gameOver = Board.hitMine(x, y)
        if Board.isWinner() and gameOver == False:
            gameOver = True
            winner = True

    print(Board)
    if winner:
        print("Congratulations, You Win!")
    else:
        print("You hit a mine, Game Over!")

playGame()