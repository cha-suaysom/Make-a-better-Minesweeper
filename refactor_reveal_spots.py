...
class boardClass(object):
    ...
    def makeSafeMove(self,x,y):
        if (self.board[x][y].selected == False):
            if self.board[x][y].value == -1:
                return False
            self.board[x][y].selected = True
            self.selectableSpots -= 1
...
def analysis(boardSize, numMines, numGames, numReveal,show=False):
    gameWon = 0
    for i in range(numGames):
        Board = boardClass(boardSize, numMines)
    #print(Board)
        for i in range(numReveal):
            x = randint(0, boardSize-1)
            y = randint(0, boardSize-1)
            Board.makeSafeMove(x,y)        
        if (show):
...
for i in range(3,10):
    boardSize = 10
    numMines = boardSize*boardSize/i
    numGames = 100
    numReveal = boardSize*boardSize/2
    ...
    show = False
    analysis(boardSize, numMines, numGames, numReveal, show)
#playGame()